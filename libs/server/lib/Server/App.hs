...
server = getWeather
    ...
    where   getWeather :: Config -> CacheState -> Location -> Time.POSIXTime -> AppM Weather
            getWeather env cacheState loc date = do
...
type WeatherAPI (prefix :: Symbol) = prefix :> "city" :> Capture "cityName" String :> "time" :> Capture "ctime" Time.POSIXTime :> Get '[JSON] Weather
